...
Route::post('/login', [LoginController::class, 'login'])->name('login.post')->middleware('throttle:5,1'); // maks 5 percobaan per menit
...
Route::middleware(['auth', LocationFilter::class])->group(function () {
    ...
    Route::group(['middleware' => ['role:super_admin,manager']], function () {
        ...
        Route::post('vehicles/{vehicle}/upload-barcode', [VehicleController::class, 'uploadBarcode'])->name('vehicles.upload-barcode');
        Route::delete('vehicles/{vehicle}/document', [VehicleController::class, 'deleteDocument'])->name('vehicles.delete-document');
        Route::post('vehicles/{vehicle}/upload-document', [VehicleController::class, 'uploadDocument'])->name('vehicles.upload-document');
        Route::post('vehicles/{vehicle}/update-expiry-dates', [VehicleController::class, 'updateExpiryDates'])->name('vehicles.update-expiry-dates');
